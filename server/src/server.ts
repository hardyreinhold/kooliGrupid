...
app.post("/createGroup", async (req: Request, res: Response) => {
  try {
    const { member, groupCode } = req.body as {
      member: string;
      groupCode: string;
    };
...
app.post("/joinGroup", async (req: Request, res: Response) => {
  const { groupCode, member } = req.body as {
    member?: string;
    groupCode?: string;
  };

  const groupsRef = db.collection("groups");
  const groupQuery = await groupsRef
...
app.post("/removePerson", async (req: Request, res: Response) => {
  try {
    const { groupId, member } = req.body as {
      groupId: string;
      member: string;
    };
